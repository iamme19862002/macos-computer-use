...
#### 截图功能
- **修复 `screenshot --app` 无法捕获应用弹窗/Sheet 的问题**
- 当应用存在多个窗口（如主窗口+弹窗）时，改为分别截取每个窗口并合成，避免仅截取主窗口导致弹窗丢失
- 使用 `CGWindowListCreateImage` 分别捕获各窗口内容，按 z-order 合成到一张图片，避免区域截图混入其他应用背景
